...
export async function GET(req: NextRequest, { params }: { params: { slug: string } }) {
  ...
  const salaries = await Salary.find(q)
    .sort({ createdAt: -1, _id: -1 })  // Sort by most recent first
    .limit(limit + 1)
    .select("_id salary jobTitle location createdAt")
    .lean();

  // Determine if there is a next page
  const hasMore = salaries.length > limit;
  const data = hasMore ? salaries.slice(0, limit) : salaries;
